...
import { IsString, IsArray, ValidateNested, IsIn, IsNotEmpty, ArrayNotEmpty, ArrayMinSize } from 'class-validator';
...
export class CreateArticleDto {
    ...
    @ApiProperty({
        ...
        example: 'Este artículo explora cómo la IA puede mejorar los procesos educativos mediante algoritmos de personalización y análisis de datos.'
    })
    @IsString()
    @IsNotEmpty()
    abstract: string;
    ...
    @ApiProperty({
        ...
        example: [
            { name: 'Juan Pérez', email: 'juan.perez@example.com', affiliation: 'Universidad Nacional de Formosa' },
            { name: 'María Gómez', email: 'maria.gomez@example.com', affiliation: 'Instituto de Tecnología Avanzada' }
        ]
    })
    @IsArray()
    ...
    @Type((obj) => {
        if (!obj || !obj.object) {
            throw new Error('No se puede determinar el tipo de articleTheme: articleType es obligatorio');
        }
        return obj.object.articleType === 'Paper Científico' ? PaperDto : BitacoraDto;
    })
